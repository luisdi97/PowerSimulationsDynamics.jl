...
function get_pss_input_signal(
    ...
    error(
        "Compensated frequency in PSS at $(PSY.get_name(dynamic_device)) not supported",
    )
end
...
function mdl_pss_ode!(
    ...
    #Compute PSS output signal and update inner vars
    inner_vars[V_pss_var] = clamp(yll_2, Vst_min, Vst_max)
    return
end
...
function mdl_pss_ode!(
    ...
    x_p16 = internal_states[16]
    x_p17 = internal_states[17]
    x_p18 = internal_states[18]
    x_p19 = internal_states[19]
    ...
    yll_2, dxp16_dt = lead_lag(yll_1, x_p16, 1.0, T3, T4)
    yll_3, dxp17_dt = lead_lag(yll_2, x_p17, 1.0, T10, T11)
    yll_4, dxp18_dt = lead_lag(yll_3, x_p18, 1.0, T12, T13)

    VPSS = clamp(yll_4, Vst_min, Vst_max)

    if PT >= PSSON
        PSY.set_hysteresis_binary_logic!(pss, 1)
    elseif PT <= PSSOFF
